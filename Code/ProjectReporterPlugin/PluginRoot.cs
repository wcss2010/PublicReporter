...
namespace ProjectReporterPlugin
{
    public class PluginRoot : IReportPluginRoot
    {
        ...
        {
            get { return "重点基础研究项目建议书填报系统（1.4版）"; }
        }
        ...
        private void initEditors()
        {
            ...
            editorMap.Add("研究成果及考核指标", new DocumentPasteEditor("研究成果及考核指标", "分类逐项列出研究成果及考核指标。研究成果形式包括演示试验/验证系统 、样机、样品/样件、理论、标准/规范、数据库/软件、工程工艺等。指标体系应系统完整。"));
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        if (MessageBox.Show("真的要导出吗?", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            ...
                            dialoga.Start(new EventHandler<CircleProgressBarEventArgs>(delegate(object thisObject, CircleProgressBarEventArgs argss)
                            {
                                ...
                                //压缩
                                PublicReporterLib.Utility.ZipUtil zu = new PublicReporterLib.Utility.ZipUtil(Path.Combine(dataDir, "项目申报书.doc"));
