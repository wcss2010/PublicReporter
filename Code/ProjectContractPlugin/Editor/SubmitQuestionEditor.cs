    ...
    public partial class SubmitQuestionEditor : BaseEditor
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (dgvDetail.SelectedRows.Count == 1)
            {
                ...
                try
                {
                    statusNum = Int32.Parse(((TiJiaoYaoQiuBiao)dgvDetail.SelectedRows[0].Tag).ZhuangTai);
                }
                catch (Exception ex) { }

                //显示编辑窗体
                FrmAddOrSubmitQuestion form = new FrmAddOrSubmitQuestion(null, statusNum + 1);
                if (form.ShowDialog() == DialogResult.OK)
                    //刷新列表
                    RefreshView();
        ...
        private void dgvDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvDetail.Rows.Count >= 1 && e.RowIndex >= 0)
            {
                ...
                else if (e.ColumnIndex == dgvDetail.Columns.Count - 2)
                {
                    ...
                    {
                        //删除数据
                        ConnectionManager.Context.table("JiShuBiao").where("BianHao='" + ((TiJiaoYaoQiuBiao)dgvDetail.Rows[e.RowIndex].Tag).BianHao + "'").delete();
