...
namespace ProjectContractPlugin.Editor
{
    public partial class ConfidentialQualificationEditor : BaseEditor
    {
        ...
        public override void RefreshView()
        {
            ...
            dgvDetail.Rows.Clear();
            List<ExtFileList> list = ConnectionManager.Context.table("ExtFileList").where("ProjectID='" + ((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).projectObj.BianHao + "'").select("*").getList<ExtFileList>(new ExtFileList());
            int index = 0;
            ...
            foreach (DataGridViewRow dgvRow in dgvDetail.Rows)
            {
                ...
                if (dgvRow.Tag != null)
                {
                    ...
                    if (dgvRow.Cells[2].Tag != null)
                    {
                        ...
                        string destFile = Path.Combine(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir, realFileName);
                        if (File.Exists(sourceFile))
                        {
                            //旧地址
                            string oldFile = Path.Combine(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir, efl.RealFileName);
                            try
                            {
                                File.Delete(oldFile);
                            }
                ...
                else
                {
                    ...
                    if (dgvRow.Cells[2].Tag != null)
                    {
                        ...
                        if (File.Exists(sourceFile))
                        {
                            ...
                            efll.ProjectID = ((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).projectObj.BianHao;
                ...
                if (e.ColumnIndex == dgvDetail.Columns.Count - 1)
                {
                    if (dgvDetail.Rows[e.RowIndex].Tag != null)
                    {
                        ...
                        if (MessageBox.Show("真的要删除吗?", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            ...
                            try
                            {
                                File.Delete(Path.Combine(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir, task.RealFileName));
                            }
                            catch (Exception ex) { }
                ...
                else if (e.ColumnIndex == 2)
                {
                    if (dgvDetail.Rows[e.RowIndex].Tag != null)
                    {
                        ...
                        else
                        {
                            try
                            {
                                Process.Start(Path.Combine(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir, task.RealFileName));
                            }
                            catch (Exception ex) { }
                        }
                    }
        ...
        {
            ((DataGridView)sender)[((DataGridView)sender).Columns.Count - 1, e.RowIndex == 0 ? e.RowIndex : e.RowIndex - 1].Value = global::ProjectContractPlugin.Resource.DELETE_28;
        }
