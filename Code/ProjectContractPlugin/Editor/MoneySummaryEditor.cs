...
namespace ProjectContractPlugin.Editor
{
    public partial class MoneySummaryEditor : BaseEditor
    {
        ...
        public override void RefreshView()
        {
            ...
            if (Directory.Exists(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir))
            {
                string[] files = Directory.GetFiles(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir);
                if (files != null)
                {
        ...
        public override void OnSaveEvent()
        {
            ...
            try
            {
                if (File.Exists(ofdUpload.FileName))
                {
                    ...
                    File.Copy(ofdUpload.FileName, Path.Combine(((ProjectContractPlugin.PluginRoot)PublicReporterLib.PluginLoader.CurrentPlugin).filesDir, FileKeyName + "_" + new FileInfo(ofdUpload.FileName).Name));
                    RefreshView();
                }
            }
            catch (Exception ex)
